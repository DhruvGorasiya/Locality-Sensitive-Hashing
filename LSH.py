...
if __name__ == "__main__":
    ...
    def solve(T: str):
        ...
        """
        # Load data from CSV files into a dictionary where keys are table names
        table_data = o.load_data()

        # Remove the target table data from the dictionary and store it separately
        target = table_data.pop(T)

        # Generate shingles for each column name in the target table
        # Shingles are substrings of length 2 for each column name
        target_table_column_name_shingles = {i: o.shingles(i, 2) for i in target}

        pairs = []  # To store pairs of similar columns
        scores = []  # To store similarity scores of the pairs

        # Compare shingles of target table columns with those of other tables
        for i in target_table_column_name_shingles:
            for k in table_data:
                for j in table_data[k]:
                    shingle_j = o.shingles(j, 2)  # Generate shingles for the column name in the current table

                    # Calculate Jaccard set containment for both directions
                    containment_1 = o.jaccard_set_containment(target_table_column_name_shingles[i], shingle_j)
                    containment_2 = o.jaccard_set_containment(shingle_j, target_table_column_name_shingles[i])

                    # Check if either containment exceeds 0.5 (indicating significant overlap)
                    if containment_1 > 0.5 or containment_2 > 0.5:
                        scores.append(max(containment_1, containment_2))
                        pairs.append((i, j, k))

        # Print the pairs of columns that have significant overlap
        print(pairs)

        for i in pairs:
            # Generate shingles for the target column data
            shingles_pair_target = set()
            for j in target[i[0]]:
                shingles_pair_target = shingles_pair_target.union(o.shingles(j, 4))  # Shingles of length 4 for column data

            # Generate shingles for the corresponding column data in the other table
            shingles_pair_table2 = set()
            for k in table_data[i[2]][i[1]]:
                shingles_pair_table2 = shingles_pair_table2.union(o.shingles(k, 4))  # Shingles of length 4 for column data

            # Generate signatures for the shingles using random hash functions
            signature_size = 200
            _hashLists = o._randomHashes(signature_size)

            # Create a 1-hot vector for the target shingles
            _1_hot_pair_target = o._1_hot(list(shingles_pair_target))
            # create signature of target table
            signature_target = o.signatures(_hashLists, _1_hot_pair_target)

            # Split the signature vector into bands
            split_vectors_pair1_target = o.split_vector(signature_target, len(signature_target) // 2)

            # Create a 1-hot vector for the shingles of the other table
            _1_hot_pair_table2 = o._1_hot(list(shingles_pair_table2))
            
            # create signature of table2
            ...
            # Reset the vocabulary for the next iteration
            o.reset_vocabs()

    # Execute the solve function for the 'patientdb' table
    solve('people') 
